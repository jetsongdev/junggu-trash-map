...
### Fixed
- 키보드로 지도 HUD, 검색, 팝업, 마커와 클러스터를 이동할 때 포커스 링이 더 명확하게 보입니다. 마커와 클러스터의 스크린리더 이름도 숫자·이모지 대신 위치명 또는 "휴지통 N개 그룹"으로 읽히며, `prefers-reduced-motion` 사용자는 지도 이동·줌과 반복 애니메이션이 줄어듭니다 (I.6 2차).
