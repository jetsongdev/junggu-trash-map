...
### Changed
- 가까운/경유 휴지통 Top-3 거리선이 색·굵기뿐 아니라 패턴 자체로도 구분되도록 정리됐습니다. 1순위는 굵은 실선, 2순위는 대시(`8 6`), 3순위는 도트(`3 5`) — 색맹·고휘도 환경에서도 순위가 한눈에 보입니다. rank 2/3 opacity와 rank 3 weight를 함께 끌어올려 라이트·다크 양 테마에서 모두 또렷하게 읽힙니다. 라이트 모드에서는 거리선 색을 sky → **slate 계열**(slate-800 / 600 / 500)로 바꿔 sky 빈 마커·UserMarker와 톤이 겹치지 않고 "경로선" 의미가 분명해지면서, 라이트 타일 위에서 충분한 콘트라스트가 나옵니다 (P2.20).
