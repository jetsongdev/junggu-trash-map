...
- 자치구 데이터를 불러오는 동안 통계 바에 "○○구 로드 중…" 인디케이터(amber 점멸 점)가 잠깐 보입니다. 첫 진입과 panning으로 새 자치구 진입 시 모두 표시되고, 로드 완료 즉시 사라집니다.
- 첫 진입 후 사용자가 지도/UI를 처음 만지는 순간(터치·마우스·휠·키) 발행 7개 자치구 데이터가 백그라운드(`requestIdleCallback`)로 합류해 panning 없이 802개를 한 번에 볼 수 있습니다 (첫 paint은 그대로 한 자치구 분량만 — Lighthouse 측정 window는 무인터랙션이라 perf 영향 0).
- 통계 바 아래에 활성 자치구가 2개 이상일 때 자치구별 마커 수를 한 줄로 보여줍니다 (`중구 59 · 서초구 83 · …`). 어느 자치구의 데이터를 보고 있는지 한눈에 파악 가능.
- 자치구 데이터가 합류할 때마다 지도 하단 가운데에 토스트로 "○○구 N개"가 잠깐(1.5초) 떴다가 사라집니다. 7개 모두 들어오면 "전체 7개 자치구 802개 휴지통 로드 완료" 토스트가 3초 동안 표시됩니다.
