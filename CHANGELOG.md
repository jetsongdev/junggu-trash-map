...
- 자치구 데이터를 불러오는 동안 화면 중앙에 spinner + "자치구 데이터 로드 중" 오버레이(현재 in-flight 자치구 이름 동봉)가 표시되고, 모든 in-flight fetch가 완료되면 즉시 사라집니다. 통계 바에도 작은 amber 점멸 인디케이터로 "N개 자치구 로드 중…" 진행 카운트가 함께 보입니다.
