...
### Added
- 휴지통 popup에 운영자가 손으로 단 위치 힌트가 한 줄로 표시됩니다. 골목 안쪽·간판 사각지대처럼 좌표만으로는 찾기 어려운 통의 진입 가이드 — 힌트가 있는 통은 popup에서 주소 위 primary로 뜨고, 그 아래에 도로명 주소가 작은 회색 글씨로 후퇴해 두 줄 모두 함께 보입니다. 힌트가 없는 통은 기존과 동일하게 주소가 primary 그대로 — 빈 슬롯이 추가로 만들어지지 않습니다. 중구 첫 출시 9건은 카카오 Local API 0~3m 반경 단일 랜드마크가 명확한 통(서소문·을지로입구·을지로6가·황학동 등)에 한정 (`public/data/hints/<district>.json`). 다른 자치구·중구 추가 통은 운영자가 점진적으로 채웁니다 (P4.3).
