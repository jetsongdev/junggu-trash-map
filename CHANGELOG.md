...
### Changed
- 다른 자치구로 이동(셀렉터 탭, 검색 결과 클릭)하면 출발지·목적지가 자동으로 초기화돼 새 구 경계 밖에 stale 화살표가 남지 않습니다. 손으로 지도를 끌어 살짝 둘러보는 경우는 그대로 유지 (P3.5).
- 키보드 `=`/`-` 또는 좌하단 ➕/➖ 버튼으로 줌할 때 출발지·목적지가 있으면 viewport 중앙이 아닌 경로 중심을 기준으로 확대/축소합니다. 출발지+목적지 둘 다 있으면 두 점의 중간점, 출발지만 있으면 출발지 기준 (P2.26).

## [0.22.0] - 2026-05-10

### Changed
- 토스트가 빠른 연속으로 발사돼도 직전 토스트를 덮지 않고 위→아래 chronological 순으로 stacking됩니다. 토스트 사이 8px 간격, position(`top`/`center`)별로 독립 stack, 동시 표시 한도는 두지 않아 모든 안내가 자기 시간만큼 살아있다 사라집니다. tap mode 배너는 별개 영역으로 그대로 동작 (P2.25).
