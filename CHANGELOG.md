...
### Added
- 휴지통 popup에 운영자가 손으로 단 위치 힌트가 한 줄로 표시됩니다. 골목 안쪽·간판 사각지대처럼 좌표만으로는 찾기 어려운 통의 진입 가이드 — 힌트가 있는 통은 popup에서 주소 위 primary로 뜨고, 그 아래에 도로명 주소가 작은 회색 글씨로 후퇴해 두 줄 모두 함께 보입니다. 힌트가 없는 통은 기존과 동일하게 주소가 primary 그대로 — 빈 슬롯이 추가로 만들어지지 않습니다. 출시 시점에는 힌트 데이터가 비어 있으며(`public/data/hints/<district>.json`), 운영자가 카카오·Naver 보면서 골목 안쪽 통부터 점진적으로 채웁니다 (P4.3).
