...
### Changed
- 토스트 위치를 화면 하단에서 화면 정중앙으로 옮기고 모달 느낌으로 다듬었습니다. 둥근 사각형(`rounded-2xl`) + 충분히 투명한 배경(info `/55`, error/emphatic `/70`) + `backdrop-blur-lg`로, 사용자가 한 번 읽고 흘려보낼 안내가 시야 중심에 잠시 머물렀다 사라지면서도 뒤의 지도가 비쳐 보입니다. error는 6초, info(첫-사용 힌트 등)는 4초, emphatic은 4~6초 노출 후 자동으로 사라지고, 지도 조작은 막지 않습니다. 첫-사용 힌트는 다른 토스트(첫 방문 onboarding, "전체 N개 자치구 로드 완료" 등)가 표시 중일 때는 발사를 건너뛰어 timing 충돌이 없습니다.
