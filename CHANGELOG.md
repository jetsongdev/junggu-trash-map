...
### Changed
- 토스트 위치를 화면 하단에서 화면 정중앙으로 옮기고 모달 느낌으로 다듬었습니다. 둥근 사각형(`rounded-2xl`) + 살짝 더 투명한 배경(info `/75`, error/emphatic `/85`) + heavier backdrop-blur로, 사용자가 한 번 읽고 흘려보낼 안내가 시야 중심에 잠시 머물렀다 사라지는 형태가 됐습니다. error는 6초, info(첫-사용 힌트 등)는 4초, emphatic은 4~6초 노출 후 자동으로 사라지고, 지도 조작은 막지 않습니다.
