...
- 자치구 데이터를 불러오는 동안 화면 중앙에 spinner + "자치구 데이터 로드 중" 오버레이가 첫 paint부터 7개 모두 로드 완료까지 끊김 없이 표시됩니다. fetch 진행 중이면 현재 in-flight 자치구 이름들, 인터랙션 대기 구간(autoFallback 5초 카운트다운)이면 "N개 자치구 추가 로드 대기…", 마무리 단계엔 "마무리 중…"으로 본문이 적응적으로 바뀝니다. 통계 바에도 작은 amber 점멸 인디케이터로 in-flight 진행 카운트가 함께 보입니다.
