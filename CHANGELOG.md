...
### Added
- 현재 보고 있는 자치구의 외곽선을 지도에 점선으로 띄우고, 폴리곤 중앙에 자치구 이름을 함께 표시합니다. 외곽선은 halo + 점선 두 겹으로 그려 라이트·다크 양 테마의 어떤 도로·배경 위에서도 명확히 보입니다(라이트 = 흰 halo + slate 점선, 다크 = 검정 halo + amber 점선). 자치구 이름은 지도 위 폴리곤 중앙에 큰 글씨로 떠서 HUD 너비를 흔들지 않고 패닝 따라 자연스럽게 이동합니다 (P3.4).
