...
### Added
- 현재 보고 있는 자치구의 외곽선을 지도에 점선으로 띄웁니다. 흰 halo + 진한 점선 두 겹으로 그려 라이트·다크 양 테마의 어떤 도로·배경 위에서도 명확히 보입니다(라이트 = 흰 halo + slate 점선, 다크 = 검정 halo + amber 점선). 셀렉터 패널을 닫아 둔 상태에서도 어느 구를 보고 있는지 한눈에 보이고, 패닝으로 다른 구에 진입하면 외곽선이 따라 옮겨갑니다 (P3.4).
- 우상단 자치구 셀렉터 칩 위에 `📍 {자치구명}` 작은 배지가 상시 표시됩니다. 셀렉터를 열지 않아도 현재 보고 있는 구를 텍스트로 확인할 수 있고, 패닝으로 다른 구로 넘어가면 라벨이 즉시 갱신됩니다 (P3.4).
