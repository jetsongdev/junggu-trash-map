...
| 파일 | 상태 |
|---|---|
| `screenshot-light-extended.png` | 라이트 + 헤더에 glass-surface 적용된 상태 (in-flow, blur source 없음) |
...
## 2 시나리오 (3차 — overlay layout, true glass)

2차에서 헤더에 glass token 박았지만 in-flow 레이아웃이라 backdrop-blur가 비춰줄 콘텐츠가 없었음 → 거의 안 보임. 사용자가 "지도가 헤더 뒤로 이어져 시각적으로 떠 있는 느낌" 명시 요청 → header (`absolute z-1020`) + section (`absolute top-[60px] z-1010`)을 `<main>` 위로 띄우고 `<main>`은 `absolute inset-0`로 viewport 전체. HUD top floating groups의 `top-2` → `top-[180px]`로 보정해 새 chrome 높이 클리어.

| 파일 | 상태 |
|---|---|
| `screenshot-light-overlay.png` | 라이트 — 지도가 viewport 전체로 확장, 헤더·검색 섹션 backdrop-blur가 실제 도로망을 흐려 보여줌 |
| `screenshot-dark-overlay.png` | 다크 — 같은 효과, alpha 0.55 통해 다크 타일 콘텐츠가 헤더 뒤로 비침 |
