...
**`screenshot.png` — 줌 14 default (cluster 상태)**
...
**`screenshot-zoom17.png` — 줌 ≥15 (개별 마커 상태)**
- 클러스터 0개, 시야 안에 502개의 개별 BinMarker (DOM 전체로는 802개) — 명동·을지로·종로 일대 휴지통이 색·이모지 그대로 노출
- 보라색 혼합 마커(🗑️♻️)와 emerald 재활용 단일 마커(♻️) 톤 그대로 — Top-3/즐겨찾기/헤드업 시각 자산은 이 줌부터 활성
