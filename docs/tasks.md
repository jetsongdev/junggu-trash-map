...
- **Phase**: 3 거의 마무리 + Phase 4 data 한 칸. P3.1a/b 머지(802 bins 클러스터링), P3.2 7개 자치구 데이터, P3.1c는 obsolete. P2.20 색맹 친화 거리선 v0.14.0 / P4.3 위치 힌트 v0.15.0 / P2.22+P2.23 모바일 툴바·HUD v0.16.0 / P2.24 ETA 인라인 v0.17.0 / P3.3 25구 셀렉터 그리드 v0.18.0. **P3.4 자치구 폴리곤 outline 머지 진행 중** (viewing district amber dashed outline, light/dark/selector-open 3장 캡처). 다음 후보: I.6(a11y 라운드), P4.1(타 종류 통), P3.3-fix1(empty-toast 스냅샷 재캡처).
...
- [x] **P3.4** 자치구 폴리곤 outline + HUD 라벨 — 현재 viewing district 영구 outline + 우상단 라벨 표시. 새 컴포넌트 `components/DistrictOutline.tsx`: react-leaflet `<GeoJSON>` 두 겹 stack(halo + inner). 다크 = 검정 halo(weight 4, opacity 0.55) + amber-400 dashed inner(weight 1.5) / 라이트 = 흰 halo(opacity 0.85) + slate-800(`#1e293b`) dashed inner. `key={code}-{halo|inner}-{tileTheme}`로 코드/테마 변경 시 강제 remount(react-leaflet `data` prop immutable). TileLayer 직후 첫 레이어로 깔려 마커·distance line·route 모두 outline 위에 살아있음. 우상단 HUD에 `📍 {자치구명}` 작은 배지 상시 표시 — 셀렉터 닫혀있어도 어느 구를 보고 있는지 한눈에. 함정: 라이트 1차 amber-700 단일 stroke 시도(`#b45309`)는 OSM 타일 베이지/도로와 톤 충돌로 거의 안 보임 → halo+inner stack이 정답. snapshot `41-district-outline/` (light · dark · selector-open 3장, 라운드 1 후 갱신).
