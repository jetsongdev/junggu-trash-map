...
- **Phase**: 3 마무리 + Phase 4 data 한 칸 + I.6 a11y 2차 + P2.25 토스트 큐 완료. P3.1a/b 머지(802 bins 클러스터링), P3.2 7개 자치구 데이터, P3.1c는 obsolete. P2.20 색맹 친화 거리선 v0.14.0 / P4.3 위치 힌트 v0.15.0 / P2.22+P2.23 모바일 툴바·HUD v0.16.0 / P2.24 ETA 인라인 v0.17.0 / P3.3 25구 셀렉터 그리드 v0.18.0 / P2.19+P2.21 첫-사용 힌트 5종 v0.20.0 / P3.4 자치구 폴리곤 outline + I.6 a11y 2차 v0.19.x / P2.24 Liquid Glass refresh v0.21.0~0.21.1 / **P2.25 토스트 stacking 큐 진행 중** (single state → queue, 최대 3장, position별 독립 stack, evict-oldest 정책). **I.6 a11y 1차 + P3.3-fix1 empty-toast 스냅샷 merged** (axe 0 violations / 42 passes — region landmark + destination aria-label 정리 2건). **I.6 a11y 2차 완료** (focus-visible ring 공통화, `=`/`-` 키보드 지도 zoom, Leaflet 마커/클러스터 스크린리더 라벨, prefers-reduced-motion에서 fly/zoom·pulse/spin 감속). **I.7 release prebump 운영 중** (prebump on PR + finalize `release:*` 라벨 게이트 운영). 다음 후보: P4.1(타 종류 통 합치기).
...
- [x] **P2.24** Liquid Glass 디자인 언어 — `globals.css` `.glass-surface` / `.glass-surface-strong` / `.glass-toast` 토큰 + light/dark 분기 + `prefers-reduced-transparency` fallback. 6 표면 적용: 좌상단 필터 / 우상단 모드 stack / 좌하단 메뉴 칩 / 검색박스+dropdown / 우하단 status 카드 / 토스트+탭 배너. 활성 칩 alpha 0.15 → 0.85로 강화해 색 identity 보존. (라운드 2: header / Leaflet popup / DistrictSelector / 헤더 테마 토글 추가 적용. 라운드 3: header(z-1020)와 search section(z-1010)을 `<main>` 위로 absolute로 띄우고 `<main>`을 `absolute inset-0`로 viewport 전체 차지 → backdrop-blur가 실제 지도 콘텐츠를 흐려 보여주는 true Liquid Glass 효과 활성. HUD top floating groups의 `top-2` → `top-[180px]`로 보정해 새 chrome 높이 클리어. fix1: ShareButton·FilterChips.INACTIVE·DistrictSelector.TRIGGER_INACTIVE에 stale `bg-white/95 ring-neutral-300` 잔존했던 부분 통일.)
...
- [x] **P3.5** 자치구 전환 시 경로 상태 초기화 — Selector 탭(populated/empty) + 검색 결과 클릭(다른 구 점프)에 `resetRouting(name)` 호출 → `userLocation`/`destination`/`tapTarget` 초기화 + watch 종료 + info 토스트(`{name}로 이동 — 출발/목적지 초기화됨`, 3s). 손 드래그(panning)는 reset 안 함 — `handleCenterChange`는 그대로 district 추적/fetch만 담당해 LocateButton GPS auto-locate가 다른 구 잡아도 origin 보존(트리거 측 명시 호출 패턴이라 자연 회피). spec: `docs/superpowers/specs/2026-05-10-p3-5-p2-26-bundle-design.md`.
