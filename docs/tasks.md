...
- [x] **P2.24** Liquid Glass 디자인 언어 — `globals.css` `.glass-surface` / `.glass-surface-strong` / `.glass-toast` 토큰 + light/dark 분기 + `prefers-reduced-transparency` fallback. 6 표면 적용: 좌상단 필터 / 우상단 모드 stack / 좌하단 메뉴 칩 / 검색박스+dropdown / 우하단 status 카드 / 토스트+탭 배너. 활성 칩 alpha 0.15 → 0.85로 강화해 색 identity 보존.
