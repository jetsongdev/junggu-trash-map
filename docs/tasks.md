...
- **markercluster `disableClusteringAtZoom`은 "그 줌부터 cluster off"**: `disableClusteringAtZoom: 15` → 줌 ≥15에서 개별, <15에서 cluster. spec의 "줌 <15 cluster / 줌 ≥15 개별" 임계와 일치. 같은 임계가 P2.10 Top-3 dimming(BinMarker rank/dimmed)과 자연 정렬됨 — Top-3 강조는 줌 ≥15에서 마커가 풀려야 시각적으로 의미 있고, 그 이하 줌에서는 어차피 cluster bubble로 묶여 보이지 않음.
